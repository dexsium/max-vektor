...
## Регистрация нового аккаунта не поддерживается

Клиент умеет только **вход в существующий аккаунт** MAX. В протоколе
реализован единственный тип запроса — `START_AUTH`, и заметки апстрима
фиксируют, что живой тест давал LOGIN-токен, а не REGISTER
(`docs/PROGRESS.md`, этап 10).

Симптом: на незарегистрированный номер сервер отвечает `cmd=1`, выдаёт
verify-токен и условия ввода кода — то есть выглядит так, будто код
отправлен, — но код не приходит.

Что делать: зарегистрировать номер в официальном приложении MAX, дальше
входить через Max Vektor. После этого код при входе обычно приходит
push-уведомлением в официальный клиент, а не отдельной SMS.

Реализовывать регистрацию «на глаз» нельзя: какие поля MAX требует при
создании аккаунта, в этом проекте не реверсили, а выдумывать поля
протокола — прямой путь к блокировке номера.
