...
`CFBundleVersion` подставляет CI — это номер запуска workflow
(`github.run_number`), он растёт на единицу с каждым пушем в
`max-vektor-ios`. Файл артефакта называется
`max-vektor-b<номер>-unsigned.ipa`, тот же номер видно в приложении:
Настройки → «О приложении» → `Max Vektor 0.1.17 (сборка 12)`. Так понятно,
какая именно сборка стоит на телефоне.

Это **наш** номер. Версия официального клиента MAX, которой приложение
представляется серверу (`MaxProto.appVersion` / `appBuild`), живёт отдельно
и с ним не связана — подставлять туда свой счётчик нельзя, см. раздел
«Сервер отвечает „Приложение устарело“».

При локальной сборке номер берётся из `pubspec.yaml` (`version: 0.1.17+18`)
или из явного `flutter build ios --build-number=N`.
...
Поддерживается: если номера ещё нет в MAX, после ввода кода приложение
спросит имя и создаст аккаунт.

Как это устроено в протоколе (источник — модуль авторизации официального
веб-клиента web.max.ru, публично отдаваемый браузеру JS):

```
op 17  {phone, type: 'START_AUTH', language, captchaToken?}   запрос кода
op 18  {token, verifyCode, authTokenType: 'CHECK_CODE'}       проверка кода
op 23  {token, tokenType: 'REGISTER', firstName, lastName?}   создание аккаунта
```

Ответ op 18 различает три исхода: `profile` — аккаунт есть, вход выполнен;
`passwordChallenge` — включён пароль 2FA; `tokenAttrs.REGISTER.token` —
аккаунта нет, этот токен идёт в op 23 вместе с именем.

Ограничение: веб-клиент перед запросом кода проходит капчу
(`captchaToken` в op 17). Android-путь, которым ходит Max Vektor, капчу
пока не требует. Если сервер её потребует, вход остановится с ошибкой:
капчу приложение не проходит и обходить её не будет.

## Код подтверждения не приходит

Симптом: сервер отвечает на запрос кода `cmd=1` с токеном и `codeLength`
(«отправлено»), но ни SMS, ни push не приходят.

Причина: **тип устройства**. Сервер по-разному относится к клиентам:

| deviceType | Капча (op 224) | Код |
|---|---|---|
| `ANDROID` | `Captcha creation disabled` | не отправляется |
| `WEB` | выдаёт ссылку на `id.vk.ru/not_robot_captcha` | приходит после прохождения капчи |
| `IOS` | не требуется | приходит сразу |

На ANDROID капча выключена, то есть «доказать», что клиент настоящий,
нечем — и код молча не отправляется. Апстрим-форк всегда слал
`deviceType: ANDROID`, поэтому вход не работал.

Max Vektor на iPhone представляется как `IOS` с полным iOS-профилем
(`pushDeviceType: APNS`, реальная модель и версия ОС) — так же, как
официальное iOS-приложение, где капчи нет. Тип соединения сохраняется
рядом с токеном (`token_kind`), потому что токен, выданный IOS-сессии,
сервер не примет от ANDROID.

На Android-сборке остаётся `ANDROID`, и там вход по номеру не работает по
той же причине. Обходной путь для Android — вкладка «У меня есть
auth-token»: взять токен из веб-версии, где капчу можно пройти руками.

## «Логин ограничен. Требуется установить 2FA»

Сервер отвечает кодом `error.profile.active.session.no2fa`: аккаунт
существует, но на нём не задан пароль двухфакторной защиты, и MAX не
пускает вход с нового устройства. Ограничение серверное, обойти его
клиентом нельзя — в официальном веб-клиенте на этот случай есть
отдельный экран `session-no-2fa`.

Что делать: в официальном приложении MAX, где вход уже выполнен, задать
пароль двухфакторной защиты (Настройки → Конфиденциальность и
безопасность), затем войти в Max Vektor заново — после кода приложение
спросит этот пароль.

## Сервер отвечает «Приложение устарело»

Симптом: вход по номеру падает, в приложении видно
`Приложение устарело, пожалуйста, обновитесь` (в старых сборках — просто
`AUTH_REQUEST cmd=3`). При этом соединение поднимается нормально.

Причина: сервер MAX сверяет версию клиента на AUTH_REQUEST (op 17).
`INIT` (op 6) при этом проходит, поэтому поломка выглядит как что угодно,
только не как версия.

Лечение — обновить `MaxProto.appVersion` в `lib/core/constants.dart` на
актуальную версию официального клиента (package `ru.oneme.app`; версию
видно на странице приложения в RuStore). Проверить, не пересобирая
приложение под iOS:

```bash
dart run tool_probe_auth.dart
```

Скрипт поднимает соединение и шлёт AUTH_REQUEST с заведомо невалидным
номером, показывая ответ сервера:

- `Приложение устарело…` — версия всё ещё старая;
- `Проверьте номер…` — версия принята, авторизация работает.

Реальный номер в скрипт подставлять не надо: на валидный номер сервер
отправит настоящую SMS, а частые запросы кода — бан-сигнал.

`MaxProto.appBuild` (versionCode) сервер не проверяет; в открытых
источниках versionCode не публикуется, поэтому там осталось значение от
26.15.0.
